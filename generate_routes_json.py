...
for stop in csv.DictReader(open('gtfs/stops.txt')):
...
routes_by_id = {}
for route in csv.DictReader(open('gtfs/routes.txt')):
    routes_by_id[route['route_id']] = route

weekday_service_ids = set(['WD'])
trips_by_id = {}

for trip in csv.DictReader(open('gtfs/trips.txt')):
...
for stop_time in csv.DictReader(open('gtfs/stop_times.txt')):
...
for xfer in csv.DictReader(open('gtfs/transfers.txt')):
